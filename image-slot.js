...
(() => {
  ...
  function lsRead() { try { const t = localStorage.getItem(LS_KEY); return t ? JSON.parse(t) : null; } catch (e) { return null; } }
  function lsWrite() { try { localStorage.setItem(LS_KEY, JSON.stringify(slots)); } catch (e) {} }
  ...
  function toast(msg, bad, sticky) {
    ...
    if (!toastEl) {
      ...
      toastEl.style.cssText = 'position:fixed;left:50%;bottom:18px;transform:translateX(-50%);' +
        ...
        'font:12px/1.3 system-ui,-apple-system,sans-serif;color:#fff;' +
        'box-shadow:0 4px 16px rgba(0,0,0,.25);transition:opacity .3s;pointer-events:none;opacity:0';
    ...
    if (!sticky) toast._t = setTimeout(() => { toastEl.style.opacity = '0'; }, 2200);
  }
  ...
  function save() {
    ...
    saving = true;
    const attempt = (n) => Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
      .then(() => { toast('Photo saved'); })
      .catch(() => {
        if (n < 4) {
          toast('Retrying photo save\u2026', true, true);
          return new Promise((res) => setTimeout(res, 800 * Math.pow(2, n)))
            .then(() => attempt(n + 1));
        }
        toast('\u26a0 Photo save FAILED \u2014 keep this tab open and drop the image again', true, true);
      });
    attempt(0).then(() => { saving = false; if (saveDirty) { saveDirty = false; save(); } });
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this.addEventListener('dragstart', (e) => {
        if (!this.id || !this.hasAttribute('data-filled') ||
            !this.hasAttribute('data-editable') || this.hasAttribute('data-reframe')) {
          e.preventDefault();
          return;
        }
